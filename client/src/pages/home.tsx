...
export default function HomePage() {
  usePageTitle("OpenRegio — Alles wat lokaal ondernemen sterker maakt.");
  const { user } = useAuth();
    ...
    <div style={{ fontFamily: FF, background: "#faf9f5", margin: 0, padding: 0 }}>
      ...
      <section style={{ background: "#fff", padding: "56px 24px" }}>
        <div className="or-usp-grid" style={{ maxWidth: 1000, margin: "0 auto", display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "20px 32px" }}>
          {[
            "Klanten en ondernemers dichtbij",
            "Workshops en activiteiten uit de buurt",
            "Sterker zichtbaar in je regio",
            "Samenwerken met lokale ondernemers",
            "Hulp bij regels, in gewone taal",
          ].map((item) => (
