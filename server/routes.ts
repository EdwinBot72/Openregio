...
[
  ...
  function parseRss(xml: string, sourceName: string): NewsItem[] {
    ...
    for (const raw of matches) {
      ...
      items.push({ id, title, summary, link, source: sourceName, publishedAt: safeIsoDate(pub), aiContext: null, related: [], media: [], lokaleImpact: [] });
    }
    return items;
  }

  async function generateNewsContext(item: NewsItem): Promise<{ aiContext: string; related: { titel: string; toelichting: string }[]; media: { naam: string; hoek: string; impact: string }[]; lokaleImpact: { sector: string; toelichting: string }[] }> {
...
Geef tot slot 3-6 CONCRETE soorten lokale ondernemingen die dit nieuws het meest gaat raken. Wees specifiek: niet "horeca" maar "kleine cafés en bruine kroegen", niet "winkels" maar "lokale slagers" of "fietsenwinkels in dorpscentra". Denk breed: detailhandel, horeca, ambachten (kapper, schoenmaker, hovenier), zorgverleners, bouw, transport, agrarisch, ZZP'ers, evenementenbureaus, etc. Per type: korte feitelijke toelichting (1-2 zinnen) hoe dit hen concreet raakt — kosten, klanten, regels, vergunningen, personeel of vraag.
  ...
  "lokaleImpact": [
    ...
    return {
      ...
      media: Array.isArray(parsed.media)
        ...
        : [],
      lokaleImpact: Array.isArray(parsed.lokaleImpact)
        ? parsed.lokaleImpact
            .filter((m: any) => m && typeof m.sector === "string")
            .slice(0, 6)
            .map((m: any) => ({
              sector: String(m.sector),
              toelichting: typeof m.toelichting === "string" ? m.toelichting : "",
            }))
        : [],
    };
  }
    ...
    try {
      ...
      const items = await Promise.all(
        newsListCache.items.map(async (item) => {
          ...
          if (cached && now - cached.ts < NEWS_CONTEXT_TTL_MS && cached.lokaleImpact) {
            return { ...item, aiContext: cached.aiContext, related: cached.related, media: cached.media, lokaleImpact: cached.lokaleImpact };
          }
          try {
            const ctx = await generateNewsContext(item);
            newsContextCache.set(item.id, { ...ctx, ts: now });
            return { ...item, aiContext: ctx.aiContext, related: ctx.related, media: ctx.media, lokaleImpact: ctx.lokaleImpact };
          } catch (err) {
...
Geef alleen de brieftekst terug, zonder commentaar, zonder markdown, zonder JSON.`;
  ...
  app.post("/api/zoektermen/genereer", (req, res) => {
    ...
    const BEROEP_DATA: Record<string, {
      label: string;
      ...
      bakker: {
        ...
        longTail: ["vers brood zaterdag {stad}", "glutenvrij brood {stad}", "biologische {beroep} {stad} oost", "croissants {stad} centrum", "zuurdesem brood {stad}"],
      ...
      fietsenmaker: {
        ...
        longTail: ["fiets reparatie {stad} aan huis", "elektrische fiets reparatie {stad}", "{beroep} open zondag {stad}", "goedkope fiets revisie {stad} oost", "fiets ophaalservice {stad}"],
        zoekvragen: ["{beroep} in de buurt {stad}?", "fiets ophaalservice {stad}?", "hoeveel kost een fiets revisie {stad}?"],
        wijkTip: "Maak een pagina per stadsdeel: /fietsenmaker-{stad}-noord, /fietsenmaker-{stad}-west. Vermeld metrostations en tramhaltes bij jou in de buurt.",
      },
      loodgieter: {
        label: "Loodgieter",
        primaireTermen: ["{beroep} {stad}", "{beroep} spoed {stad}", "CV ketel storing {stad}", "lekkage reparatie {stad}"],
        longTail: ["{beroep} 24 uur {stad}", "{beroep} Scheveningen", "verstopt toilet {stad} centrum", "waterleiding repareren {stad}", "erkende installateur {stad}"],
        zoekvragen: ["{beroep} met spoed nodig {stad}?", "wat kost een {beroep} in {stad}?", "erkend installateur {stad}?"],
        wijkTip: "Spoedtermen scoren het hoogst. Zet 'spoed' en '24 uur' prominent op je homepage. Maak een aparte spoedpagina met je telefoonnummer bovenaan.",
      },
      kapper: {
        label: "Kapper",
        primaireTermen: ["{beroep} {stad}", "{beroep} {stad} centrum", "heren {beroep} {stad}", "dames {beroep} {stad}"],
        longTail: ["{beroep} {stad} kralingen afspraak", "goedkope {beroep} {stad} zuid", "{beroep} open zondag {stad}", "balayage {stad} {beroep}", "{beroep} kind {stad} noord"],
      ...
      restaurant: {
        ...
        wijkTip: "Vermeld aanloopstraten en bezienswaardigheden dichtbij: 'op 2 minuten van het station' of 'naast het stadspark'. Dit helpt bij navigatiezoekopdrachten.",
      },
      elektricien: {
        label: "Elektricien",
        ...
        wijkTip: "Vermeld je VCA/ISSO-certificaten en erkend installateur status op elke pagina — dit verhoogt vertrouwen én SEO-relevantie voor professionele zoekopdrachten.",
      },
      fysiotherapeut: {
        ...
        wijkTip: "Maak specialisatiepagina's: rugklachten, sportblessures, zwangerschapsklachten. Lokale zoekopdrachten zijn vaak specifiek op klacht — niet alleen op beroep.",
      },
      schilder: {
        label: "Schilder",
        ...
        wijkTip: "Fotogalerij per project met buurtvermelding werkt goed: 'Project Jordaan — voordeur en kozijnen'. Dit genereert organisch lokale zoekwoordcombinaties.",
      },
    };

    const data = BEROEP_DATA[beroep];
    if (!data) {
      return res.status(400).json({ error: "onbekend beroep" });
    }

    const fill = (s: string) =>
      s.replace(/{beroep}/g, beroep).replace(/{stad}/g, stad).replace(/{wijk}/g, wijk ?? stad);

    res.json({
      primair: data.primaireTermen.map(fill),
      longTail: data.longTail.map(fill),
      zoekvragen: data.zoekvragen.map(fill),
      paginatitel: `${data.label} ${stad}${wijk ? " " + wijk : ""} | [Jouw bedrijfsnaam]`,
      metaDescription: `Uw ${beroep} in ${stad}${wijk ? " " + wijk : ""}. ${fill(data.primaireTermen[1] ?? "")} — Bel direct voor een afspraak.`,
      h1Suggestie: `${data.label} in ${stad} — [Jouw slogan of specialiteit]`,
      wijkTip: data.wijkTip,
    });
  });
  ...
  app.post("/api/seo-checklist/:userId", requireAuth, async (req, res) => {
    ...
    try {
      ...
      console.error("[SeoChecklist] save error:", err);
