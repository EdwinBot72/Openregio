...
export class MemStorage implements IStorage {
  ...
  async createUser(userData: { email: string; passwordHash: string; plan?: "pending" | "basic" | "pro" | "coaching"; role?: "member" | "master" | "admin"; firstName?: string | null; lastName?: string | null; mustCompleteOnboarding?: boolean; onboardingToken?: string | null; referralCode?: string | null; referredByUserId?: string | null; referredAt?: Date | null }): Promise<User> {
    ...
    const user: User = {
      ...
      emailLokaleActiesDigest: false,
      lastDigestSentAt: null,
      lastLokaleActiesSeenAt: null,
      createdAt: new Date(),
      updatedAt: new Date(),
      deletedAt: null,
    };
  ...
  async upsertUser(userData: UpsertUser): Promise<User> {
    ...
    const user: User = {
      ...
      lastNewsReadAt: existing?.lastNewsReadAt || null,
      emailNewsDigest: existing?.emailNewsDigest ?? true,
      emailLokaleActiesDigest: existing?.emailLokaleActiesDigest ?? false,
      lastDigestSentAt: existing?.lastDigestSentAt || null,
      lastLokaleActiesSeenAt: existing?.lastLokaleActiesSeenAt || null,
